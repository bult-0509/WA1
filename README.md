...
首页提供五个专题入口，侧栏也可按编号切换。日期范围、日期类型和站点筛选即时生效；站点留空表示全部。空间页还可看进出总量，并切换到5月1日至12日的10分钟动画。动画固定使用附件中的9个有效日，进入该视图后不适用的侧栏筛选会禁用。天气固定用全网进站量，聚类固定用四个月有效数据。
...
地图使用发布方坐标和邻接关系，无在线底图；坐标参考系未声明，连线不是精确轨道。10分钟动画的边流量来自后续提供的预生成附件，附件没有生成公式，不能解释为真实OD、方向客流或去重人数。天气为城市代表点，不是逐站观测；相关不证明因果。三类出行由发布方推断。聚类评价是现有数据的内部评价，不能当作预测准确率。
