...
软件开发实践1，第1题。使用MetroFlow公开聚合数据和2017年上海POI，分析302站的时间高峰、空间分布、出行类别、天气关联及周边城市功能，另用KMeans对站点曲线分组。
...
2. 双击“一键运行.bat”（便携演示包也可使用“启动.vbs”或“启动.cmd”）。无需安装Python，无需联网，无需管理员权限。
...
| 站点空间 | 地理站点图、前20站日均排名和10分钟边流量动画 |
| 出行模式 | 进出方向、出行构成、站点聚类三个视图 |
| 天气关联 | 温度/雨量散点、雨日比较、相关系数及样本数 |
| POI与客流 | 500米/1公里站点圈、设施密度与多样性、客流相关、四分位组和偏离站点 |
| 数据质量 | 来源、字段、真实检查结果、六个缺损日和处理前后对比 |

首页提供六个专题入口，侧栏也可按编号切换。日期范围、日期类型和站点筛选即时生效；站点留空表示全部。空间页还可看进出总量，并切换到5月1日至12日的10分钟动画。动画固定使用附件中的9个有效日。天气固定用全网进站量，聚类固定用四个月有效数据。POI页固定比较全部站点，可调整客流日期、日期类型和进出方向。
...
- 系统界面内置阿里妈妈方圆体，字体来源与声明见 `static/fonts/README.md`；离线运行无需安装字体。
...
源码包自带处理后数据，可直接运行七页系统；只有重新清洗数据才需要原始文件。
...
POI原文件为`2017上海市POI_全部.csv`，约296.8MB，不进入Git或代码ZIP。重新生成POI汇总时执行：

```powershell
.venv\Scripts\python.exe prepare_poi_data.py --input "D:\数据\2017上海市POI_全部.csv"
```

脚本分块读取1,625,290行数据，按名称、地址、坐标和细分类移除370条候选重复记录；剔除仅作地图标注的地名地址、通行、室内及道路附属设施后，保留1,303,199个功能POI。MetroFlow站点坐标先由WGS84转为GCJ-02：转换后与“地铁站”POI的最近距离中位数由476.3米降至15.7米，302站中298站在300米内匹配到地铁站POI。花桥、光明路位于昆山，不在上海POI覆盖范围内。
...
地图使用发布方坐标和邻接关系，无在线底图，连线不是精确轨道。10分钟动画的边流量来自后续提供的预生成附件，附件没有生成公式，不能解释为真实OD、方向客流或去重人数。POI采用1公里直线站点圈，范围允许重叠；功能多样性为九类功能的归一化Shannon指数。天气与POI关联都只说明统计关系，不证明因果。三类出行由发布方推断。聚类评价是现有数据的内部评价，不能当作预测准确率。
...
prepare_poi_data.py    POI分块清洗与站点圈汇总入口
build_portable.py      双包构建
launch.py              简单本地启动与关闭
一键运行.bat           Windows一键启动入口
src/                   数据处理、统计、聚类、图表
data/raw/              原始CSV（不进交付包）
data/processed/        小时/日汇总、辅助数据、分组结果
assets/                本地图标等
static/fonts/           本地界面字体及来源说明
static/metro_map/       10分钟边流量动画及本地Leaflet
tests/                 统计、质量和页面检查
...
- **缺数据或无法读取**：重新完整解压；开发机按准备说明运行prepare_data.py及prepare_poi_data.py，答辩机不现场下载。
- **筛选后空白**：仅选择六个异常日会显示无有效数据提示，扩大日期范围即可。
- **中文路径或字体**：路径按文件所在位置计算，支持中文和空格；界面字体随包加载，失败时回退到Windows微软雅黑。图中源站名保留发布方英文名称及ID。
