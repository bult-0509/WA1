...
const POI_DATA='POI数据共1625290条；排除地址、通行、室内和道路附属信息后，1303199条进入功能分析。';
...
function chart(s,type,categories,values,name,pos,extra={}){const c=s.charts.add(type,{position:pos,categories,series:[{name,values,fill:C.teal,line:{fill:C.teal,width:3}}],hasLegend:false,chartFill:C.bg,plotAreaFill:C.bg,xAxis:{textStyle:{fontSize:20,color:C.muted,typeface:'Microsoft YaHei'}},yAxis:{textStyle:{fontSize:18,color:C.muted,typeface:'Microsoft YaHei'},majorGridlines:{fill:'#DFE6EA',width:1,style:'solid'}},dataLabels:{showValue:true,position:'outEnd',textStyle:{fontSize:22,color:C.ink,typeface:'Microsoft YaHei'}},...extra});[c.legend?.textStyle,c.dataLabels?.textStyle,c.xAxis?.textStyle,c.yAxis?.textStyle].filter(Boolean).forEach(v=>{v.typeface='Microsoft YaHei';});return c;}
...
{
const s=slide('六个问题决定功能范围','每个页面都要回答问题，并给出可核验的结果。');
['A  高峰何时出现？','B  哪些站点更繁忙，在哪里？','C  早晚进出方向如何变化？','D  三类出行各占多少？','E  天气与客流是否有关？','F  站点周边功能与客流有何关系？'].forEach((v,i)=>text(s,v,72,230+i*66,1080,52,31,i%2?C.ink:C.teal,true));
...
{
const s=slide('把数据处理和页面展示分开','五个小模块支撑七个页面，便于复核和答辩解释。');
text(s,'客流、天气、站点与POI → 检查、校正和聚合',70,270,1140,62,37,C.teal,true);
text(s,'统计、关联与聚类 → 本地加载 → 筛选、解释与导出',70,375,1140,62,35,C.ink,true);
...
{
const s=slide('站点周边POI越密集，客流通常也越高','1公里范围采用功能类POI，相关关系不等于因果。');
await shot(s,'S12-poi-analysis-top.png');
points(s,['密度与进站量\nSpearman ρ = 0.627','95%自助法区间\n0.539—0.698','高密度组客流中位数\n是低密度组的3.30倍'],850,246,365,27,126);
notes(s,'约50秒。先将地铁站WGS84坐标转换为GCJ-02，再按球面距离统计500米和1公里范围POI。用383条地铁站POI验证后，中位坐标偏差从476.3米降到15.7米。密度与客流呈中等偏强正相关；功能多样性与客流相关较弱，区间跨零。高客流站就业金融占比较高，购物占比较低，但这里只报告关联，不做因果判断。'+POI_DATA+'花桥、光明路位于昆山，上海POI数据覆盖不到，因此周边计数为0。','本地实际截图：docs/screenshots/S12-poi-analysis-top.png\n本地结果：data/processed/poi_stations.parquet、poi_quality.json\n本地源码：src/poi_processing.py、src/analysis.py');
}
{
const s=slide('相对曲线聚成2组，内部评价为0.427','初始参照为3组；统一比较2—5组后选择最高轮廓系数。');
chart(s,'bar',['2组','3组','4组','5组'],[0.427,0.299,0.260,0.205],'轮廓系数',{left:65,top:250,width:730,height:385},{yAxis:{numberFormatCode:'0.0',textStyle:{fontSize:18,color:C.muted,typeface:'Microsoft YaHei'},majorGridlines:{fill:'#DFE6EA',width:1,style:'solid'}},series:[{name:'轮廓系数',values:[0.427,0.299,0.260,0.205],valuesFormatCode:'0.000',fill:C.teal,dataLabelOverrides:[0.427,0.299,0.260,0.205].map((v,idx)=>({idx,text:v.toFixed(3),showValue:false,textStyle:{fontSize:22,color:C.ink,typeface:'Microsoft YaHei'}}))}]});
...
{
const s=slide('一次演示：筛选、定位、解释、导出','七页系统共用清楚的统计口径。');
await shot(s,'S01-overview.png');points(s,['概览 → 时间 → 站点','出行模式 → 天气 → POI','质量核对与结果导出'],850,270,360,28,125);
notes(s,'预留约75秒现场演示：先看时间规律，再进入站点空间选择站点，切换出行模式，展示天气和POI关联，最后查看质量与导出。不要在答辩时重新训练模型。若现场演示未能完成，可用本页真实截图解释，但不能声称异机已通过。','本地实际截图：docs/screenshots/S01-overview.png\n本地源码：app.py');
}
{
const s=slide('把可运行与可解释一起交付','开发验证有记录，现场验收仍要实做。');
text(s,'19项测试通过',70,265,1100,70,47,C.teal,true);
text(s,'清洗边界、统计口径、七页、POI处理、筛选及空状态',70,348,1120,60,30);
text(s,'源代码ZIP小于30MB；独立便携包内置Python与数据',70,463,1120,58,30);
...
text(s,'成果',70,265,220,55,35,C.teal,true);text(s,'时间与空间规律、方向与构成、天气和POI关联、站点分组',300,265,900,88,31);
text(s,'特色',70,400,220,55,35,C.teal,true);text(s,'离线交互、共同有效日期、清楚的质量说明与AI留档',300,400,900,88,31);
text(s,'限制',70,535,220,55,35,C.teal,true);text(s,'历史样本、单点天气、POI覆盖边界；关联结论不能代替因果分析',300,535,900,100,31);
